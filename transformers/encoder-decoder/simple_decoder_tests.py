...
class TransformerDecoder(nn.Module):
    def __init__(self, vocab_size: int, d_model: int, num_layers: int, num_heads: int, max_len: int = 512, d_ff: int = 2048):
        super().__init__()
...
def test_masked_multihead_self_attention():
    d_model = 32
    num_heads = 4
    seq_len = 6
    batch_size = 2
    x = torch.randn(batch_size, seq_len, d_model)
    attn = MaskedMultiHeadSelfAttention(d_model, num_heads)
    out = attn(x)
    assert out.shape == (batch_size, seq_len, d_model)
    print("[✓] MaskedMultiHeadSelfAttention:", out.shape)
...
def test_cross_attention():
    ...
    out = cross_attn(x, encoder_output)
...
def test_transformer_decoder():
    ...
    encoder_output = torch.randn(batch_size, src_seq_len, d_model)
    decoder = TransformerDecoder(vocab_size, d_model, num_layers, num_heads, d_ff=d_ff)
    out = decoder(tgt, encoder_output)
    assert out.shape == (batch_size, tgt_seq_len, vocab_size)
    print("[✓] TransformerDecoder:", out.shape)
...
if __name__ == "__main__":
    ...
    test_masked_multihead_self_attention()
    test_cross_attention()
    test_decoder_block()
    test_transformer_decoder()

'''
[✓] PositionalEncoding: torch.Size([2, 5, 16])
[✓] MaskedMultiHeadSelfAttention: torch.Size([2, 6, 32])
[✓] CrossAttention: torch.Size([2, 6, 32])
[✓] DecoderBlock: torch.Size([2, 6, 64])
[✓] TransformerDecoder: torch.Size([2, 10, 1000])
'''